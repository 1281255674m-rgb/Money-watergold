...
import { INVITATION_STORAGE_KEY, WelcomeInvitationDialog } from "./WelcomeInvitationDialog";
...
describe("WelcomeInvitationDialog", () => {
  ...
  it("shows after a short delay and stays dismissed for seven days", () => {
    ...
    expect(localStorage.getItem(INVITATION_STORAGE_KEY)).toBeTruthy();
    first.unmount();

    const second = render(<WelcomeInvitationDialog {...props} />);
    act(() => vi.advanceTimersByTime(900));
    expect(second.container.querySelector("dialog")).not.toHaveAttribute("open");
  });
